...
def solve_k4(m, budget=10000, seed=42):
            ...
            for start in range(n):
                if not vis[start]:
                    ...
                    while not vis[cur]:
                        ...
                        # Table idx: i, j, k_ (fiber-structured)
    ...
    for it in range(budget):
        ...
        if it % 100000 == 0 and it > 0:
            print(f"  it={it} score={cur_sc} best={bs} T={T:.6f}")
    return None

if __name__ == "__main__":
    print("Searching for Z_2^4 stratified solution (size 8)...")
    sol2 = solve_k4(2, budget=100000, seed=42)
    if sol2: print(f"Z_2^4 SOL: {sol2}")

    print("\nSearching for Z_4^4 stratified solution (size 64)...")
    sol4 = solve_k4(4, budget=2000000, seed=123)
    if sol4: print(f"Z_4^4 SOL: {sol4}")
