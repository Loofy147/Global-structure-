...
# ══════════════════════════════════════════════════════════════════════════════
# SIMULATED ANNEALING — full 3D sigma
# ══════════════════════════════════════════════════════════════════════════════

def sa_search(m: int,
              max_iter:  int   = 800_000,
              T_init:    float = 4.0,
              T_min:     float = 0.005,
              seed:      int   = 0,
              verbose:   bool  = True,
              report_n:  int   = 100_000) -> Tuple[Optional[SigmaMap], dict]:
    """
    Simulated annealing on full 3D sigma.
    Score = Σ (components_c - 1). Want 0.
    Perturbation: change sigma at one random vertex.
    Returns (sigma_or_None, stats_dict).
    """
    rng   = random.Random(seed)
    verts = vertices(m)
    n     = m**3
    ...
    # Init
    sigma = {v: rng.choice(_ALL_PERMS) for v in verts}
    cur_s = score(sigma, m)
    best_s= cur_s
    best  = {v:p for v,p in sigma.items()}
    ...
    for it in range(max_iter):
        ...
        if verbose and (it+1)%report_n==0:
            elapsed = time.perf_counter()-t0
            print(f"  {D}iter={it+1:>8,}  T={T:.4f}  "
                  f"score={cur_s}  best={best_s}  "
                  f"elapsed={elapsed:.1f}s{Z}")

    elapsed = time.perf_counter()-t0
    stats = {"iters": it+1, "best_score": best_s, "elapsed": elapsed,
             "improvements": improvements, "final_T": T}
...
def sa_multistart(m: int,
                  restarts:  int   = 8,
                  iter_each: int   = 600_000,
                  T_init:    float = 4.0,
                  verbose:   bool  = True) -> Tuple[Optional[SigmaMap], dict]:
    """
    Run SA from multiple random starting points.
    Return first success, or best attempt.
    """
    best_score = 999; best_sigma = None; total_iters = 0; t0 = time.perf_counter()
    for restart in range(restarts):
        if verbose:
            print(f"\n  {D}SA restart {restart+1}/{restarts}  seed={restart}{Z}")
        sigma, stats = sa_search(m, max_iter=iter_each, seed=restart,
                                  verbose=verbose, report_n=200_000,
                                  T_init=T_init)
        total_iters += stats["iters"]
        if sigma is not None:
            elapsed = time.perf_counter()-t0
            return sigma, {"restarts_used": restart+1, "total_iters": total_iters,
                           "elapsed": elapsed, "best_score": 0}
        if stats["best_score"] < best_score:
            best_score = stats["best_score"]; best_sigma = sigma
    elapsed = time.perf_counter()-t0
...
def phase_04(fast: bool = False):
    ...
    if sigma4 is None:
        miss(f"SA did not reach score=0 in this budget")
        miss(f"Best score: {stats['best_score']}  (need 0)")
        note("Increase restarts or iter_each for more search time")
        note("The solution EXISTS (Knuth paper confirms m=4 is solvable)")
        return {"m": 4, "found": False, "stats": stats}

    # ── FOUND ─────────────────────────────────────────────────────────────────
    found(f"m=4: VALID solution found in {stats['elapsed']:.1f}s!")
